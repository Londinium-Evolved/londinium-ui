 ...
 */
async function processTiff(tiffData: ArrayBuffer, targetResolution: { width: number, height: number }) {
  try {
    ...
    self.postMessage({
      type: TerrainWorkerMessageType.ERROR,
      data: `Failed to process TIFF: ${error.message}`
    });
  }
}
